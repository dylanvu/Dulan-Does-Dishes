...
const RecipeEditor = ({ recipe }: { recipe: Recipe | undefined }) => {
    ...
    useEffect(() => {
        // if there are recipes, populate the states with them
        if (recipe) {
            changeRecipeName(recipe.name);
            changeRecipeSteps(recipe.steps);
            changeIngredientsList(recipe.ingredients);
            if (recipe.background) {
                changeBackground(recipe.background)

            }
            if (recipe.postCooking) {
                changePostCooking(recipe.postCooking);

            }
            if (recipe.rating) {
                changeRating(recipe.rating);
            }

            changePictures([recipe.img]);
        } else {
            console.log("Brand new recipe")
        }
        ...
        getAllTags().then((tags) => {
            if (tags) {
                ...
                const tagButtons: tagButton[] = tags.map((tag) => {
                    ...
                    // if we are bringing in tags already, make those selected
                    if (recipe) {
                        // if it's included, select it
                        if (recipe.tags[tag.name]) {
    ...
    return (
        <div>
            ...
            <TextAreaInput value={recipe && recipe.rating ? rating : undefined} title="Rating" placeholder={recipe && recipe.rating ? recipe.rating : "What did you think of the dish?"} changeTextAreaState={changeRating} />
            <TextAreaInput value={recipe && recipe.background ? background : undefined} title="Background" placeholder={recipe && recipe.background ? recipe.background : "What's the background? Who or what inspired you? Why did you make this?"} changeTextAreaState={changeBackground} />
            <TextAreaInput value={recipe && recipe.postCooking ? postCooking : undefined} title="Post-Cooking Remarks" placeholder={recipe && recipe.postCooking ? recipe.postCooking : "What happened during or after cooking? Any other random information?"} changeTextAreaState={changePostCooking} />
